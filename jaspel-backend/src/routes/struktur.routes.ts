...
app.get("/", async (c) => {
  ...
  const items = await db.select().from(strukturOrganisasi).orderBy(strukturOrganisasi.urutan);
  const allPegawai = await db.select({ id: pegawai.id, nama: pegawai.nama }).from(pegawai);
